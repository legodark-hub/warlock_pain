...
*   **API-сервер (FastAPI):** Предоставляет HTTP-эндпоинт для взаимодействия с чат-ботом, позволяя интегрировать его с другими приложениями.
...
3. Вводите свои сообщения и нажимайте Enter, чтобы общаться с Пейном.
4. Чтобы выйти из чата, введите `выход` или `quit`.
...
1. Запустите FastAPI-сервер с помощью Uvicorn:
    ...
    * `api_server`: Имя Python-файла, содержащего FastAPI-приложение (`api_server.py`).
    * `fastapi_app`: Имя экземпляра FastAPI в файле `api_server.py`.
    * `--reload`: Флаг для автоматического перезапуска сервера при изменениях в коде (удобно для разработки).
2. Сервер будет доступен по умолчанию на `http://127.0.0.1:8000`.
3. Вы можете взаимодействовать с API через эндпоинт `/chat`.
...
* **Тело запроса (JSON):**
    ...
    ```json
    {
      "thread_id": "string",  // Уникальный идентификатор для отслеживания истории диалога
...
* **Ответ (JSON):**
    ...
    ```json
    {
      "character_name": "string", // Имя персонажа
      "response": "string"        // Ответ персонажа
    }
    ```

* Вы также можете получить доступ к интерактивной документации API (Swagger UI) по адресу `http://127.0.0.1:8000/docs` в вашем браузере после запуска сервера.
